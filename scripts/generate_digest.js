...
const CONTENT = {
  ...
  layer2: {
    regional: [
      ...
      ["Telangana / Hyderabad", "Alongside the ongoing legal battle (government appeal of the removal order, the separate Malkajgiri contempt proceeding, five IAS/IPS contempt notices, Section 22A land-freeze designations already on file), a distinct institutional-growth signal has emerged: representatives from several Telangana districts met Commissioner Ranganath directly on 12 Aug to formally request HYDRAA's mandate be expanded statewide to fight lake/land encroachments, rather than remain confined to the Hyderabad core region.", "A push for statewide expansion — arriving even as HYDRAA's leadership faces an active legal challenge — suggests grassroots/district-level institutional demand for its enforcement model may outlast the current commissioner dispute. If HYDRAA's mandate does expand beyond Hyderabad, NBR should treat any Telangana land diligence outside the current TCUR jurisdiction as a forward-looking watch item, not just a Hyderabad-specific concern."],
